    ...
    fun seedData(connection: Connection) {
        ...
        if (!tableHasData(connection, "dining_places")) {
            connection.createStatement().executeUpdate(
                ...
                ('barbara-s-heritage','Barbara''s Heritage Restaurant','Heritage dining with cultural performances.','A popular heritage dining hall known for Filipino set menus and occasional cultural showcases near key plazas.','Restaurant','Filipino','General Luna Street, Intramuros','10:00 AM - 9:00 PM','PHP 500-1,200','(02) 8527 3893','Reserve ahead on weekends.',14.5894,120.9750,'',TRUE,'open',1),
                ('cafe-intramuros','Cafe Intramuros','Relaxed heritage cafe for breaks.','A quiet cafe for coffee, rice meals, and snacks close to museums and churches.','Cafe','Filipino Comfort','Plaza San Luis, Intramuros','8:00 AM - 8:00 PM','PHP 200-500','(02) 8536 1120','Good brunch stop before museum hours.',14.5899,120.9740,'',TRUE,'open',2),
        ...
        connection.createStatement().use { st ->
            ...
            st.executeUpdate("UPDATE dining_places SET image_path='', image_urls='[]'")
            st.executeUpdate("UPDATE local_services SET image_path='', image_urls='[]'")
            st.executeUpdate("UPDATE tour_routes SET image_url='', image_urls='[]'")
        }
    }
